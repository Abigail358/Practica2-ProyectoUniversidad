...
public class MateriaController {
    ...
    @PutMapping("/{materiaId}/asignar-docente/{docenteId}")
    @Transactional
    @ResponseStatus(HttpStatus.OK) 
    public ResponseEntity<MateriaDTO> asignarDocenteAMateria(
            @PathVariable Long materiaId,
            @PathVariable Long docenteId) {
        MateriaDTO materiaActualizada = materiaService.asignarDocenteAMateria(materiaId, docenteId);
        return ResponseEntity.ok(materiaActualizada);
    }

    @GetMapping("/por-docente/{docenteId}")
    public ResponseEntity<List<MateriaDTO>> obtenerMateriasPorDocente(
            @PathVariable Long docenteId) {
